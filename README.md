...
- so this is "SHARED WRAPPER"
  ...
  In Summary - if DFT adds wrapper then "DEDICATE WRAPPER" if DFT used flops functionally present in the design as wrapper cell then "SHARED"
  Ports registered - then flop is already present then "SHARED"
  Ports not registered - then either add "DEDICATE WRAPPER" or after some combo logic use the flop functional present as wrapper then it is "SHARED"

$[FIX_ME] to add diagram for above</n>
$[FIX_ME] to add pros & cons for above from timing point of view & from ATPG point of view (there is a trade off) 
